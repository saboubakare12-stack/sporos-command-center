...
// --- Market config cache ---
let marketConfigCache = { data: null, timestamp: 0 };
const CONFIG_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

// Yahoo Finance uses different symbols for some indices
const SYMBOL_MAP = {
  '^SPX': '^GSPC',
};
...
/**
 * Load market config from Google Sheet.
 * Sheet columns: Symbol | Category | Display Name
 * Categories: Index, Mag 7, Sector ETFs, Zacks Rank 1
 */
export async function loadMarketConfig() {
  const now = Date.now();
  if (marketConfigCache.data && now - marketConfigCache.timestamp < CONFIG_CACHE_TTL) {
    return marketConfigCache.data;
  }

  const client = getSheets();
  if (!client) return null;

  const sheetId = process.env.GOOGLE_SHEET_ID_MARKET || process.env.GOOGLE_SHEET_ID_TASKS;
  if (!sheetId || sheetId === 'your_task_sheet_id_here') return null;
  ...
  try {
    ...
    for (const row of rows.slice(1)) {
      const rawSymbol = (row[symIdx] || '').trim();
      if (!rawSymbol) continue;

      const category = (row[catIdx] || '').trim().toLowerCase();
      const displayName = (row[nameIdx] || rawSymbol).trim();
      const symbol = mapSymbol(rawSymbol);

      const entry = { symbol, name: displayName };

      if (category === 'index') {
        indices.push(entry);
      } else if (category === 'mag 7') {
        watchlist.push(entry);
      } else if (category === 'sector etfs') {
        sectors.push(entry);
      } else if (category === 'zacks rank 1') {
        zacksRankOne.push(entry);
      }
    }

    const config = { indices, watchlist, sectors, zacksRankOne };
    marketConfigCache = { data: config, timestamp: now };
    return config;
  } catch (err) {
    console.error('Error loading market config from sheet:', err.message);
    return null;
  }
}

// GET /api/sheets/market-config — Return market config from Google Sheet
router.get('/market-config', async (_req, res) => {
  try {
